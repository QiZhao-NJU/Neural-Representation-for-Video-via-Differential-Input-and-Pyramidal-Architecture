...
@torch.no_grad()
def evaluate(model, dataloader, device, cfg, args, Epoch, save_images=False):
	...
	for i, data in enumerate(dataloader):
		...
		if (Epoch + 1) % 300 ==0 :
			for batch_ind in range(1):
				full_ind = i * 1 + batch_ind #i*1 + 0
				
				save_dir = visual_dir+ '/E' + str(Epoch)
				if not os.path.isdir(save_dir):
					os.makedirs(save_dir)
				save_image(output_list[-1][batch_ind], save_dir+'/pred_'+'{:05d}'.format(full_ind)+'.png')
